...
- **多 Agent 隔离记忆** — 支持多个 Agent 并行运行（dev / blog / pjm / pm / prototype / researcher 等），各 Agent 记忆空间完全隔离、互不干扰。自动扫描发现所有有 workspace 的 Agent，无需手动注册。标记为 `experience` 的记忆自动在所有 Agent 间共享，沉淀团队集体经验。
