...
- **短期 + 长期分层存储** — 对话以日记文件形式捕获并每天提炼为短期记忆；Agent 精心维护的 `MEMORY.md` 直接同步到长期记忆。完整链路：实时会话 → 日记快照 → 每日 LLM 提取 → 向量记忆。
...
- **全自动运维** — systemd timer 全程自动化：每 5 分钟会话快照、UTC 01:00 MEMORY.md 同步、UTC 01:30 日记 digest、UTC 02:00 短期记忆归档。零人工干预，服务重启后自动恢复。
