...
def trim_diary_if_oversized(path: Path, agent_id: str) -> bool:
    """日记大小保护：超过 MAX_DIARY_BYTES 时按字节裁剪，循环直到满足限制。

    保留文件头（# 标题 + ## Session 记录 段落），再从尾部保留尽可能多的内容，
    确保最终文件大小 <= MAX_DIARY_BYTES。
    ...
    try:
        ...
        trim_notice = (
            f"\n\n> ⚠️ 日记超过 {MAX_DIARY_BYTES // 1024}KB 上限，已自动裁剪。"
            f" 裁剪时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}\n\n"
        )
        prefix = header + trim_notice
        prefix_bytes = len(prefix.encode('utf-8'))

        # 按字节预算从尾部保留内容，循环确保不超限
        budget = MAX_DIARY_BYTES - prefix_bytes - 1  # -1 for trailing newline
        tail_lines = lines[header_end:]
        while tail_lines:
            candidate = '\n'.join(tail_lines) + '\n'
            if len(candidate.encode('utf-8')) <= budget:
                break
            # 每次丢弃前 10% 的行（至少 1 行），加速收敛
            drop = max(1, len(tail_lines) // 10)
            tail_lines = tail_lines[drop:]
        ...
        logger.info(
            ...
            f"(kept last {len(tail_lines)} lines)"
        )
        return True
    except Exception as e:
...
def _msg_hash(line: str) -> str:
    """计算消息行的短 hash，用于持久化去重（不受 trim 裁剪影响）"""
...
def write_to_memory(messages: list, path: Path, agent_id: str, session_key: str = "") -> tuple[int, list[dict]]:
    """写入 session 消息到 memory 文件，基于 offsets.json 中的消息 hash 集合去重。

    去重机制：每条消息行计算 hash，与 offsets.json 中该 session 已记录的 hash 集合比对，
    只写入未出现过的消息。写入成功后追加新 hash 到集合。
    这样即使 trim 裁剪了日记文件内容，去重仍然有效。
    ...
    try:
        ...
        # 写入成功后，把新消息 hash 追加到 offsets 持久化
        new_hashes = [_msg_hash(all_lines[i]) for i in new_indices]
        written_hashes.update(new_hashes)
        if session_key not in offsets:
            offsets[session_key] = {}
        offsets[session_key]["written_hashes"] = list(written_hashes)
