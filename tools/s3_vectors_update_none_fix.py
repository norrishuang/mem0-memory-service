...
class S3Vectors(VectorStoreBase):
    ...
    def update(self, vector_id, vector=None, payload=None):
        ...
        # When vector=None (e.g. event=NONE metadata-only update), fetch the existing
        # vector data first so we don't pass None to put_vectors (boto3 validation error).
        if vector is None:
            existing = self.get(vector_id)
            if existing is None:
                logger.warning(f"update called with vector=None but {vector_id} not found; skipping")
                return
            # Fetch vector data (need returnData=True)
            try:
                ...
                if vector is None:
                    ...
                    return
                # Use existing payload if new payload not provided
                if payload is None:
